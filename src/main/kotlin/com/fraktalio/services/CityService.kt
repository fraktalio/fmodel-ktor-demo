...
class CityService(private val dataSource: DataSource) {
    companion object {
    ...
    suspend fun create(city: City): Int = withContext(Dispatchers.IO) {
        val statement = dataSource.connection.prepareStatement(INSERT_CITY, Statement.RETURN_GENERATED_KEYS)
    ...
    suspend fun read(id: Int): City = withContext(Dispatchers.IO) {
        val statement = dataSource.connection.prepareStatement(SELECT_CITY_BY_ID)
    ...
    suspend fun update(id: Int, city: City) = withContext(Dispatchers.IO) {
        val statement = dataSource.connection.prepareStatement(UPDATE_CITY)
        statement.setString(1, city.name)
        statement.setInt(2, city.population)
    ...
    suspend fun delete(id: Int) = withContext(Dispatchers.IO) {
        val statement = dataSource.connection.prepareStatement(DELETE_CITY)
        statement.setInt(1, id)
        statement.executeUpdate()
    }
}